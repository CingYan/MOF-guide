...
V.questTimeline = async () => {
  ...
  const onlyOpen = el('input', { type: 'checkbox', 'aria-label': '只看未完成' });
  const summary = el('p', { class: 'lead' });
  const openStorageKey = 'mof-quest-timeline-open-v1';
  const loadOpenState = () => {
    try { return new Map(Object.entries(JSON.parse(localStorage.getItem(openStorageKey) || '{}'))); }
    catch (_) { return new Map(); }
  };
  const saveOpenState = () => {
    const state = Object.fromEntries([...list.querySelectorAll('details[data-timeline-key]')]
      .map(node => [node.dataset.timelineKey, node.open]));
    localStorage.setItem(openStorageKey, JSON.stringify(state));
  };
  list.addEventListener('toggle', event => {
    if (event.target.matches('details[data-timeline-key]')) saveOpenState();
  }, true);
  ...
  const draw = () => {
    ...
    list.querySelectorAll('details[data-timeline-key]').forEach(node => openState.set(node.dataset.timelineKey, node.open));
    localStorage.setItem(openStorageKey, JSON.stringify(Object.fromEntries(openState)));
    ...
    ordered.forEach(([area, areaQuests]) => {
      ...
      orderedStages.forEach(([stageKey, batch], index) => {
        ...
        batch.forEach(q => {
          (q.hunt || []).forEach(x => {
            ...
            o.variants.set(x.target.id, (o.variants.get(x.target.id) || 0) + (Number(x.count) || 0));
          });
          (q.collect || []).forEach(x => {
      ...
      const addHuntGroup = (monster, q, kind, objective) => {
        ...
        const key = baseName(monster.name);
        if (!huntGroups.has(key)) huntGroups.set(key, { monster, variants: new Map(), tasks: new Map(), maps: new Set() });
        const group = huntGroups.get(key);
        group.variants.set(monster.id, monster);
        if (!group.tasks.has(q.id)) group.tasks.set(q.id, { q, kinds: new Set(), objectives: [] });
        const task = group.tasks.get(q.id);
        task.kinds.add(kind); task.objectives.push({ kind, objective });
        (monster.maps || []).forEach(map => map.name && group.maps.add(map.name));
      };
      areaQuests.filter(q => !(q.indun || []).length).forEach(q => {
        (q.hunt || []).forEach(x => addHuntGroup(monsterById.get(x.target?.id), q, '討伐', x));
        (q.collect || []).forEach(x => (drops.get(x.target?.id) || []).forEach(m => addHuntGroup(m, q, '蒐集', x)));
      });
      const huntBatchList = el('div', { class: 'quest-timeline-hunt-batches' });
      [...huntGroups.entries()]
        .sort((a, b) => {
          const interval = group => {
            const tasks = [...group.tasks.values()];
            const levels = tasks.map(x => Number(x.q.levelReq) || 0);
            const huntLevels = tasks.filter(x => x.kinds.has('討伐')).map(x => Number(x.q.levelReq) || 0);
            return {
              first: Math.min(...levels, 999),
              last: Math.max(...(huntLevels.length ? huntLevels : levels), 0),
            };
          };
          const aa = interval(a[1]), bb = interval(b[1]);
          return aa.first - bb.first || aa.last - bb.last || a[0].localeCompare(b[0], 'zh-Hant');
        })
        .forEach(([name, group]) => {
          const tasks = [...group.tasks.values()].sort((a, b) => (a.q.levelReq || 0) - (b.q.levelReq || 0) || a.q.name.localeCompare(b.q.name, 'zh-Hant'));
          const hunts = tasks.filter(x => x.kinds.has('討伐'));
          ...
          huntBatchList.appendChild(el('details', { class: 'quest-timeline-hunt-batch', 'data-timeline-key': `hunt:${area}:${name}`, open: isOpen(`hunt:${area}:${name}`) }, [
            ...
            el('section', { class: 'quest-timeline-stage' }, [
              ...
              el('p', { class: 'quest-timeline-meta', text: `對應任務：${taskNames}` }),
            ]),
          ]));
        });
      ...
      if (dungeonQuests.length) {
        ...
        dungeonQuests.filter(q => !onlyOpen.checked || !done[q.id]).sort((a, b) => (a.levelReq || 0) - (b.levelReq || 0) || routeDepth(a) - routeDepth(b)).forEach(q => {
          const c = el('input', { type: 'checkbox', checked: !!done[q.id], 'aria-label': `標記任務 ${q.name} 完成` });
          c.onchange = () => { done[q.id] = c.checked; localStorage.setItem('mof-quest-route-done', JSON.stringify(done)); draw(); };
          const conditions = conditionNodes(q);
          const task = el('li', { class: 'quest-timeline-task' }, [el('label', {}, [c, el('a', { href: '#/quests/' + q.id, text: q.name }), el('span', { class: 'dim', text: `（Lv.${q.levelReq || 0}｜${(q.npcs || []).map(n => n.name).join('、') || '無 NPC'}）` })]),
            conditions.length ? el('span', { class: 'quest-timeline-condition' }, ['條件：', ...conditions.flatMap((x, i) => [i ? '、' : '', x])]) : el('span', { class: 'quest-timeline-condition dim', text: '條件：副本任務動作' }),
            ...previousText(q).map(t => el('span', { class: 'quest-timeline-meta', text: t }))]);
          const npcs = (q.npcs || []).map(n => n.name).join('、') || '未記錄 NPC';
          dungeonContent.appendChild(el('details', { class: 'quest-timeline-stage-fold quest-timeline-dungeon', 'data-timeline-key': `dungeon:${area}:${q.id}`, open: isOpen(`dungeon:${area}:${q.id}`) }, [
            el('summary', { class: 'quest-timeline-stage-summary', text: `Lv.${q.levelReq || 0}｜${q.name}` }),
            el('section', { class: 'quest-timeline-stage' }, [el('strong', { class: 'quest-timeline-step', text: '① 先接取' }), el('ol', { class: 'quest-route-tasks' }, [task]), el('strong', { class: 'quest-timeline-step', text: '② 進入副本並完成任務' }), el('p', { class: 'quest-timeline-meta', text: (q.indun || []).map(x => `副本：${x.dungeon || '未記錄'}${x.entryItem?.name ? `｜入場：${x.entryItem.name}` : ''}`).join('；') }), el('strong', { class: 'quest-timeline-step', text: '③ 回報並解鎖後續' }), el('p', { class: 'quest-timeline-meta', text: `完成後回報：${npcs}。` })]),
          ]));
        });
        if (dungeonContent.querySelector('.quest-timeline-dungeon')) content.appendChild(dungeonContent);
      }
  ...
  root.append(el('h1', { text: '任務流程' }), el('p', { class: 'sub', text: '本頁以任務鏈時間軸為主體，先按等級分段，再依前置排列：先接取 → 執行同批任務 → 回報 → 解鎖下一批。副本任務另列獨立流程；任務路線頁則以怪物為主軸，適合查同一怪物的集中狩獵。' }), el('div', { class: 'filters quest-route-filters' }, [el('label', {}, ['目前 Lv.', fromLevel]), el('label', {}, ['目標 Lv.', toLevel]), search, el('label', {}, [onlyOpen, '只看未完成'])]), summary, list);
  draw(); return root;
};
...
const NAV_GROUPS = [
  [['', '首頁'], ['monsters', '怪物'], ['maps', '地圖'], ['equips', '裝備'],
              ...
              ['quests', '任務'], ['quest-timeline', '任務流程'], ['npcs', 'NPC'], ['pets', '寵物']],
...
const ROUTE = {
  ...
  items: V.items, recipes: V.recipes, quests: V.quests, 'quest-plan': V.questPlan, 'quest-timeline': V.questTimeline, npcs: V.npcs,
